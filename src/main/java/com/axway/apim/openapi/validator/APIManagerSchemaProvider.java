...
public class APIManagerSchemaProvider {
    ...
    private String getFrontendAPISpec(String apiId) throws Exception {
        try (CloseableHttpResponse httpResponse = apiManager.get("/discovery/swagger/api/id/" + apiId + "?swaggerVersion=2.0")) {
            int statusCode = httpResponse.getStatusLine().getStatusCode();
            String response = EntityUtils.toString(httpResponse.getEntity());
            if (statusCode != 200) {
                if (statusCode == 500 && response.contains("Failed to download API")) {
                        ...
                        try (CloseableHttpResponse openApiResponse = apiManager.get("/discovery/swagger/api/id/" + apiId + "?swaggerVersion=3.0")) {
                            statusCode = openApiResponse.getStatusLine().getStatusCode();
                            response = EntityUtils.toString(openApiResponse.getEntity());
                            if (statusCode == 200) {
                                return response;
                            }
                        }
